...
func findMinDistPoints(n int, points []tools.Point3D) []tools.PointsPair {
	...
	for i := 0; i < n; i++ {
		...
		pointsPairs = append(pointsPairs, tools.PointsPair{P1: minPoint1, P2: minPoint2})
	}
	return pointsPairs
}
...
func calcRes(content string, minPointsCnt int) (int64, error) {
	...
	minDistPoints := findMinDistPoints(minPointsCnt, points)
	for _, pointsPair := range minDistPoints {
